...
class RefactorExample(wx.Frame):
    ...
    def __init__(self,parent,id,size):
        wx.Frame.__init__(self,parent,id,"PDF合并",style = wx.CAPTION|wx.CLOSE_BOX|wx.FRAME_SHAPED,size = size)
        panel = wx.Panel(self,-1)
        panel.SetBackgroundColour("White")
        self.Bind(wx.EVT_CLOSE,self.OnCloseWindow)
        self.createMenuBar()
        #打开文件按钮
        self.OpenButton = wx.Button(panel,-1,"打开文件",size = (80,30),pos = (10,10))
        self.Bind(wx.EVT_BUTTON,self.OnOpenButton,self.OpenButton)
        #选择输出文件夹按钮
        self.OutButton = wx.Button(panel,-1,"输出文件夹",size = (80,30),pos = (100,10))
        self.Bind(wx.EVT_BUTTON,self.OnOutButton,self.OutButton)
        #输出文件夹显示框
        self.OutText = wx.TextCtrl(panel,-1,size=(230,30), pos=(190,10))
        #调试信息显示框
        self.Inform = wx.TextCtrl(panel,-1,size=(320,260), pos=(10,50),
                                  style = wx.TE_MULTILINE)
        #开始合并按钮
        self.StartButton = wx.Button(panel,-1,"开始合并",size = (80,30),pos = (340,50))
        self.Bind(wx.EVT_BUTTON,self.OnStart,self.StartButton)
    def menuData(self):
        return (("&文件",("&打开","&Open in status",self.OnOpen),("&退出","Quit",self.OnCloseWindow)),
            ("&编辑",("&复制","Copy",self.OnCopy),("&剪切","Cut",self.OnCut),("&粘贴","Paste",self.OnPaste),
    ...
    def OnOpenButton(self,event):
        ...
        self.Inform.AppendText(os.path.basename(self.InFile[self.InFileNum-1])+"\n")
